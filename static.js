...
const { parseUrl, walk, getContentType, fileExt } = require("genart-lo");
...
function isEncoding(name, type, headers) {
  ...
  headers["Content-Type"] =
    getContentType(fileExt(name.replace(/\.([^.]*)$/, ""))) || "";
}

function toHeaders(name, stats, isEtag) {
  let headers = {
    "Content-Length": stats.size,
    "Content-Type": getContentType(fileExt(name)) || "",
...
module.exports = function (dir, opts = {}) {
  ...
  if (!opts.dev) {
    walk.sync(dir, (name, abs, stats) => {
      ...
      if (opts.cache && typeof opts.cache === "function") {
        let _cc = opts.cache(name, abs, stats);
        if (typeof _cc === "number") _cc = `max-age=${_cc}`;
